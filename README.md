...
- 每次都会逐个预检订阅链接，跳过不可转换的来源，再检查最终 YAML 中的 `proxies`、`proxy-groups` 和 `rules`。日志只显示来源序号、计数和脱敏诊断。
- 只要至少一个订阅可用，工作流就会用可用来源发布；内容没有变化时不会写入 Gist。所有订阅失效、聚合转换、校验或发布失败时，Gist 会保留上一份可用配置。
